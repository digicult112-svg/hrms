-- FIX RPC AMBIGUITY
-- We explicitly DROP the function (and any overloads) before recreating it.

-- 1. Drop the function with all potential signatures to clear conflicts.
DROP FUNCTION IF EXISTS public.generate_payroll_batch(jsonb, integer, integer);
DROP FUNCTION IF EXISTS public.generate_payroll_batch(jsonb[], integer, integer);

-- 2. Recreate the Robust Function
...
BEGIN
    ...
    LOOP
        ...
        IF rec->>'base_salary' IS NOT NULL AND (rec->>'base_salary')::NUMERIC > 0 THEN
             ...
             -- Auto-save this to the master table for next time
             -- Since we are SECURITY DEFINER, we bypass the RLS check that failed the client
